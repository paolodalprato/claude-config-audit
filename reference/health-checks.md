...
## Environment asymmetries

The three Desktop environments load MCP servers from different sources
(see the environments table in SKILL.md). A server visible in one
environment and absent from another is not necessarily broken — check
which config file defines it before classifying.
